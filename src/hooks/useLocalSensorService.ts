...
export const fetchLocalRadioData = async (): Promise<SensorReading | null> => {
  try {
    ...
    console.log(`DEBUG: Fetched sensor_data.json, found ${lines.length} lines total.`);
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      try {
        const latest = JSON.parse(line);
        console.log(`DEBUG: Parsing line model ->`, latest.model);
        if (latest.model === "Vevor-7in1") {
          console.log("DEBUG: Found valid Vevor packet!", latest);
          return {
    ...
    console.warn("Radio data fetch error:", e);
    return null;
  }
};